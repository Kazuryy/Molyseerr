...
/// Horizontal scrolling row of movie genre cards
/// Displayed as a slider in the DiscoverView, matching Seerr web app
struct MovieGenresRow: View {
    let title: String
    @StateObject private var viewModel = MovieGenresRowViewModel()
...
@MainActor
class MovieGenresRowViewModel: ObservableObject {
    ...
    func loadGenres() async {
        ...
        do {
            genres = try await SeerrService.shared.getMovieGenres()
            hasLoaded = true
            print("✅ MovieGenresRow: Loaded \(genres.count) genres")
        } catch {
            print("❌ MovieGenresRow: Failed to load genres: \(error)")
...
#Preview {
    NavigationStack {
        ZStack {
            ...
            MovieGenresRow(title: "Movie Genres")
        }
    }
